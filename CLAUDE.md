...
npm run migrate:supabase # 1회성 Airtable → Supabase 데이터 이관 (tsx 스크립트)
```
...
> Airtable은 Phase 3에서 완전 폐기. `lib/airtable-helpers.ts`와 `scripts/migrate-airtable-to-supabase.ts`만 1회 이관용으로 잔존.
...
└── airtable-helpers.ts    # legacy, migrate 스크립트에서만 사용
...
- `artists.id` = Airtable 호환 `rec...` (PRIMARY KEY, 모든 FK 기준)
...
# Legacy (migrate 스크립트에서만 필요, 운영 앱에는 불필요)
AIRTABLE_TOKEN=
AIRTABLE_BASE_ID=
```
