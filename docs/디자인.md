...
Geist → **Pretendard Static Subset (Korean) 4 weight** — Regular 400 / Medium 500 / SemiBold 600 / Bold 700. `next/font/local` woff2 번들(~130KB), `display: swap`, `preload: true`. `size-adjust`/`ascent-override` 디스크립터로 CLS 최소화.
...
- **공정 노출 규칙 (rotation)**:
  ...
  - 정렬 가중치: `1 / (EXTRACT(EPOCH FROM (now() - COALESCE(last_shown_at, '2000-01-01'))) + 1)` 의 역순(=오래 노출 안 된 작가 우선).
...
- `@theme`에 `--breakpoint-pc: 800px` + `.container-primary { max-width: 1400px }` 추가 (§3.5/§3.6 정책)
- Pretendard 4 weight 로딩
- `ArtistCard.tsx` 추출, 페이지 3곳 교체
- `lib/constants/services.ts`·`regions.ts` 이전
- 마이그레이션 007 + 008 + 009 (`last_shown_at`) 적용
- 작가 편집 "대표 이미지 지정" UI + "testimonials 3건 입력" UI
- 이벤트 1~5 + 14 prod 발사
- **도구 일원화 hard cut (2026-04-23 결정, +2~3일)**:
  - 전 페이지 `alert()` → Sonner `toast.{success,error,info}` 치환 (`/artist-upload`, `/video-upload`, `/checklist`, `/artist-calendar` 등)
  - `MultiSelectDropdown`/`SingleSelectDropdown` (`/artist-register` 내부) → Radix Select 기반 `app/components/ui/Select.tsx` 단일 컴포넌트로 이전. `/search` 검색 폼의 region/service/price 셀렉트 동시 교체
  - 하드코딩 hex 일괄 치환 — §2.4 매핑 표 기준 + `/search`·`/artist-*`·`/checklist`·`/tips`에서 추가 hex 식별 후 Sprint 1 종료 시점 0건 보장 (ESLint custom rule로 회귀 차단)
...
**Phase 0 DoD**: GA4 또는 PostHog 중 택1, 1~5·14 설치. Sprint 2에 6~13. Sprint 2 후반 §6.0 출시 시 15 추가.
...
- API 계약 (§26):
  ...
  → { ok: true, artists: ArtistRow[], total: number, dbError?: boolean }
...
- API 계약 (§26):
  ```
  GET /api/artists/[id]
  → { ok: true, ...artist, video_portfolio_items: [...], updated_at: string, dbError?: boolean }
...
- 상태 매트릭스:
  ...
  | Auth 401 | redirect /login | redirect /login |
  | 403 | N/A | N/A |
  | 409 | N/A | N/A |
...
- 상태 매트릭스:
  ...
  | 상태 | UI |
  |---|---|
  | Loading | N/A — localStorage 즉시 |
  | Empty | 6 섹션 모두 unchecked로 시작 |
  | Error | toast.error만 (페이지 자체는 robust) |
  | dbError | N/A — 서버 의존 없음 |
  | Auth/403/404/409 | N/A — 공개 |
...
- 상태 매트릭스: 정적 페이지 — Loading/Empty/Error/dbError/Auth/403/404/409 모두 N/A.
...
- API 계약 (§26):
  ...
  → 200: { ok, artist: {...}, dbError? }
  → 409: { ok: false, error: { code: "UNIQUE_KAKAO" | "UNIQUE_EMAIL", message } }
  → 400: { ok: false, error: { code: "VALIDATION", fields: { fieldName: msg } } }
  ```
...
- 상태 매트릭스:
  ...
  | 409 (PATCH 동시성) | toast.warning "다른 기기에서 변경됨" + 새로고침 버튼 |
...
- 상태 매트릭스:
  ...
  | 403 | redirect /unauthorized |
  | 404 | not-found.tsx |
  | 409 OptLock | Dialog "내 변경 덮기 / 새로고침" |
...
- API 계약 (§26):
  ...
  → 200: { ok, artist, dbError? }
  → 409: { ok: false, error: { code: "OPT_LOCK", current_updated_at } }
  ```
...
```ts
type ApiResponse<T> =
  | { ok: true; dbError?: boolean; } & T
  | { ok: false; dbError?: boolean; error: { code: string; message: string; fields?: Record<string, string> } };
```
- `ok`: success/failure boolean (단일)
- `dbError`: Supabase 일시 장애 신호. true 시 글로벌 banner trigger.
- `error.code`: machine-readable (UPPER_SNAKE — `VALIDATION`, `UNIQUE_KAKAO`, `OPT_LOCK`, `INTERNAL`, ...)
