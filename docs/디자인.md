...
- **카테고리 8종 대표 아이콘**: **자체 라인 일러스트**로 제작. Phase 1 내 2일 일러스트 스프린트.
  ...
  - 카테고리 매핑: 본식스냅·서브스냅·영상촬영·아이폰스냅·돌스냅·야외스냅·스튜디오·해외촬영·커플스냅.
...
- **§20 작가 CTA 배너** (메인 섹션 #4, §6.0 직후 1줄 분리 배너) + 이벤트 #16 `artist_cta_banner_clicked`
...
- 이벤트 #2~5·#7~9·#15·#16·#17 설치 (§15.5 일정)
- **Pretendard subset 최적화 spike** 0.5d (목표 < 400KB, §3.1)
...
### Phase Next — 리뷰·에디터픽 (v3 명명 "Phase 4" → v4 합동 리뷰 후 "Phase Next" 통일)
- `reviews` 테이블 정식화 + 작성 UI
- 카드 ★ 평점 복귀 (데이터 정직 원칙 충족 이후)
- "이번 주 새 포트폴리오" → "DayPic 추천" (에디터 큐레이션)
- `price TEXT` deprecated 제거
- (§28에서 추가 항목): 신부 비교 패널 / 작가 통계 상세 / 검색 무한 스크롤 / Sentry / Lighthouse CI / size-limit / pricing tier
- (§향후): Supabase RLS (NextAuth → Supabase JWT 서명 교환)
...
- **Sprint 1b** (Refactor + 측정): #1 `page_view` 검증, #13 `artist_main_image_set` (대표 이미지 UI 동시)
...
- **Sprint 4** (신부 보조 + Error Family): #18 `mypage_view` (mypage Skeleton + 폴리모픽 정리와 동시), #19·#20 (체크리스트), #21~#23 (login_*), `not_found_viewed`, `error_boundary_triggered`
...
@persona  방문자/신부 · @sprint S1a·S1b·S2·S3 (단계 분산: foundation→토큰→Hero/카테고리/쇼케이스/CTA 배너→이번 주/무드/POINT/FAQ) · @priority P0
@file     `app/page.tsx`
...
- §6.0 직후 §20 작가 CTA 배너 (섹션 #4, 모바일 80px / PC 64px) — **Sprint 2 백로그** (§6.0 쇼케이스와 동시 구현, §12 Sprint 2 항목)
- §10 메인 섹션 순서표는 합동 리뷰 후 이미 갱신 완료 (작가 CTA 배너 #4 삽입, 배경 리듬 재조정)
...
- [ ] **§29 e2e**: /mypage 폴리모픽 분기 / /checklist autosave / /login fail toast / Error Family 4종 노출 시나리오 prod CI 통과
- [ ] **§29 contract test**: 모든 mutating endpoint가 §26 envelope `{ ok, dbError?, error?, data?: T }` 준수
...
- 이벤트 #24~33 (작가측 **10개** 신규: #24·25·26 register_form_*, #27·28 dashboard_*, #29 portfolio_uploaded, #30·31 closed_date_*, #32 video_slot_saved, #33 profile_field_changed)
...
- [ ] 통계 API LCP < 2.5s
- [ ] **§29 e2e**: /artist-register fail/success path / autosave conflict (409) / calendar 키보드 / 모바일 탭바 활성 탭 추적 — 5개 작가 라우트 prod CI 통과
- [ ] **§29 a11y**: P0 페이지 axe critical 0 / Lighthouse a11y ≥ 95
- [ ] **§29 size-limit**: §27 budget 회귀 차단 작동
- [ ] 회고 + Phase Next 백로그 (§28 명명 일관 §28 마지막 라인)

### Phase Next (v3 "Phase 4" + v4 합동 리뷰 추가 항목 통합)
- 신부 측: 비교 패널 (찜한 작가 N명 가로 테이블) + 후기 작성 UI (`reviews` 테이블 + 카드 ★ 복귀)
- 작가 측: 통계 상세 (일별 노출 그래프) + pricing tier 도입
- 검색 측: 무한 스크롤 + 정렬 옵션 (가격↑↓, 인기순) + "DayPic 추천" 에디터 큐레이션
- 시스템: PostHog 세션 녹화 + Hotjar / Supabase RLS (NextAuth → Supabase JWT 서명 교환, service_role 의존 축소)
- 인프라: Sentry + Lighthouse CI + size-limit 차단 + chaos monkey
- 데이터: `price TEXT` deprecated 제거

**총 공수 (v4 합동 리뷰 후)**: Phase 0 (5d) + Sprint 1a (1주) + Sprint 1b (2주) + Sprint 2·3 (4w) + Sprint 4·5 (4w) + 버퍼 ≈ **11~12주 (~2.7~3 인월)** (§12 정합).
...
|---|---|---|---|---|---|---|---|---|
| 22.0 / | 방문자/신부 | S1a·S1b·S2·S3 | ✅ | — | §4~10 위임 | ✅ | ✅ | §20 배너 S2 |
| 22.1 /search | 신부 | S2 | ✅ 1253L | /api/search | ✅ | ✅ + #17 신규 | 10 cell | L1019/1133 |
| 22.2 /artists/[id] | 방문자/신부 | S3 | ✅ | /api/artists/[id] | ✅ | ✅ ★ | 8 cell | Lightbox 교체 |
| 22.3 /mypage | 신부/작가 | S4 | ✅ | /api/me | ✅ × 2뷰 | ✅ + #18 | 9 cell × 2뷰 | 작가 뷰 신설 |
| 22.4 /checklist | 신부 | S4 | ✅ 436L | (localStorage) | ✅ | ✅ + #19,20 | 8 cell (dbError·Auth·403·404·409 모두 N/A) | alert→toast |
| 22.5 /tips | 방문자/신부 | S4 | ✅ 206L | — | ✅ | ✅ | 8 cell 전부 N/A (정적) | 콘텐츠 감사 |
...
| 22.9 /artist-upload | 작가 | S5 | ✅ | /portfolio-images, /representative-image | ✅ | ✅ + #29 | 9 cell (Empty 기존/새 분리, 409 N/A) | dropzone 신설 |
| 22.10 /artist-calendar | 작가 | S5 | ✅ | /artist-closed | ✅ | ✅ + #30,31 | 8 cell | ARIA grid + 키보드 |
| 22.11 /video-upload | 작가 | S5 | ✅ | /video-portfolio | ✅ | ✅ + #32 | 8 cell (409 N/A) | 4 슬롯 wizard |
| 22.12 /artist-profile/edit | 작가 | S5 | ✅ 신규 설계 완료 | PATCH /artists/[id] | ✅ | ✅ + #33 | 8 cell incl. 409 (§22 내 유일 실 동작) | autosave + 409 |
...
상태 매트릭스 (PO 섹션 **8 상태**: Loading/Empty/Error/dbError/Auth401/Forbidden403/NotFound404/OptLock409): /search·/checklist·/tips·/login은 일부 N/A 명시. 작가 라우트는 모두 401/403 redirect 명시. **§22 내 OptLock 409 실 동작은 /artist-profile/edit 단 1곳** — /artist-upload·/video-upload는 사실 정정 후 N/A (단순 upsert / updated_at bump). /search·/artists/[id]·/checklist·/tips·/mypage·/login·/artist-register·/artist-dashboard·/artist-calendar는 모두 N/A.
...
| 28 | Sprint 4 + 5 + Phase Next 백로그 | ~80L |
...
### D.12 Sprint 매핑 정합 정정 (2026-04-25)
- §28 Sprint 5 백로그 "이벤트 #24~33 (작가측 9개)" → 실제 10개로 정정 (산술 오류)
- #18 `mypage_view` 등록 일정 충돌 해소: §15.5 의 Sprint 1b 등록 → Sprint 4 등록으로 일치 (mypage 자체가 Sprint 4 작업)
- §28 마지막 "총 공수 9~10주 (~2.3 인월)" → "11~12주 (~2.7~3 인월)" 갱신 (Sprint 1a/1b 분할 반영, §12 정합)
- §22.0 메인 anchor 메타 갱신: `S1·S2·S3` → `S1a·S1b·S2·S3`. §20 작가 CTA 배너 작업 위치 명시 (Sprint 2)
- §12 Sprint 2 백로그에 §20 작가 CTA 배너 + Pretendard subset 최적화 spike 추가 (이전 누락)
- §28 Sprint 4·5 DoD 보강: §29 e2e 시나리오 통과 + contract test (envelope 준수) + a11y axe + size-limit 회귀 차단 cross-link
- Phase 명명 일관성: `Phase 4 (차기)` (§12) + `Phase 6 (차기)` (§28) + `Phase 5 RLS` (§향후) 혼재 → **`Phase Next` 단일화** + 항목 통합
- 양방향 매핑 정합 검증 결과: 13 페이지 모두 정확히 1 sprint 슬롯 (S2:1 / S3:1 / S4:5 / S5:6) — 누락·중복 0건

### D.13 부록 C 점검표 정합 정정 (2026-04-25, cycle 5)
4 사이클 변경 누적 후 부록 C cell 들이 §22.x 본문과 stale 상태였음. 8 cell 정정:
- 22.0 @sprint: `S1·S2·S3` → `S1a·S1b·S2·S3` (Sprint 1 분할 §12 정합)
- 22.1 상태 cell: `8 cell` → `10 cell` (Loading 초회/필터변경 + Empty 필터/전체 분리)
- 22.3 상태 cell: `8 cell × 2뷰` → `9 cell × 2뷰` (Cycle 1에서 404 행 추가)
- 22.4 상태 cell: `7 cell (dbError N/A)` → `8 cell` (Cycle 1에서 4 상태 분리)
- 22.5 상태 cell: `정적 (전 N/A)` → `8 cell 전부 N/A (정적)` (표 변환)
- 22.9 상태 cell: `8 cell` → `9 cell` (Empty 기존/새 분리 + 409 N/A)
- 22.11 상태 cell: 409 N/A 명시 보강
- 22.12 @file: `✅ (미상세)` → `✅ 신규 설계 완료` (실제로는 신규 설계 완료)
- 본문 마지막 "7 상태" → "**8 상태**" + 409 실 동작 페이지 명시 (1곳) + N/A 페이지 9곳 명시
